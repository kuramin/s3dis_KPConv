...
class ModelTrainer:
    ...
    def train(self, net, training_loader, val_loader, config):
        ...
        for epoch in range(config.max_epoch):
            ...
            print("Lets eval")
            net.eval()
            print("Eval done. Lets validate")
            self.validation(net, val_loader, config)
            print("Val done. Lets train")
            net.train()
    ...
    def cloud_segmentation_validation(self, net, val_loader, config, debug=False):
        ...
        print("Before saving pots config.saving is", config.saving)
        # Saving (optionnal)
        if config.saving:
            ...
            if val_loader.dataset.use_potentials:
                ...
                for i, file_path in enumerate(files):
                    ...
                    pot_name = join(pot_path, cloud_name.strip(".ply") + time.strftime('_%Y-%m-%d_%H-%M-%S', time.gmtime()))
        ...
        print("Before saving clouds config.saving is", config.saving, "and (self.epoch + 1) % config.checkpoint_gap =", (self.epoch + 1) % config.checkpoint_gap)
        ...
        if config.saving and (self.epoch + 1) % config.checkpoint_gap == 0:
            ...
            for i, file_path in enumerate(files):
                ...
                cloud_name = file_path.split('/')[-1]
                val_name = join(val_path, cloud_name.strip(".ply") + time.strftime('_%Y-%m-%d_%H-%M-%S', time.gmtime()))

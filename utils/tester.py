...
class ModelTester:
    ...
    def cloud_segmentation_test(self, net, test_loader, config, num_votes=50, debug=False):
        """
            ...
            if last_min + 1 < new_min:
                ...
                if int(np.ceil(new_min)) % 10 == 0:
                    ...
                    for i, file_path in enumerate(test_loader.dataset.files):
                        ...
                        time_and_date = time.strftime('_%Y-%m-%d_%H-%M-%S', time.gmtime())
                        # Save plys
                        cloud_name = file_path.split('/')[-1]
                        test_name = join(test_path, 'predictions', cloud_name.strip(".ply") + time_and_date)
                        write_ply(test_name,
                                  [points, preds],
                                  ['x', 'y', 'z', 'preds'])
                        test_name2 = join(test_path, 'probs', cloud_name.strip(".ply") + time_and_date)
                        ...
                        pot_name = join(test_path, 'potentials', cloud_name.strip(".ply") + time_and_date)
